...
#define BIT2BYTE(bit)    ( ((!!(bit&4))<<23) | ((!!(bit&2))<<15) | ((!!(bit&1))<<7) )
                    case 0:
                        psvDebugScreenSetFgColor(COLOR_DEFAULT_FG);
                        psvDebugScreenSetBgColor(COLOR_DEFAULT_BG);
                        break;
                    case 3:
                        psvDebugScreenSetFgColor(BIT2BYTE(params[j] % 10));
                        break;
                    case 9:
                        psvDebugScreenSetFgColor(BIT2BYTE(params[j] % 10) | 0x7F7F7F7F);
                        break;
                    case 4:
                        psvDebugScreenSetBgColor(BIT2BYTE(params[j] % 10));
                        break;
                    case 10:
                        psvDebugScreenSetBgColor(BIT2BYTE(params[j] % 10) | 0x7F7F7F7F);
                        break;
#undef BIT2BYTE
                }
            }
...
int psvDebugScreenInit() {
    ...
    SceUID displayblock = sceKernelAllocMemBlock("display", SCE_KERNEL_MEMBLOCK_TYPE_USER_CDRAM_RW, SCREEN_FB_SIZE,
                                                 NULL);
...
int psvDebugScreenPuts(const char *text) {
    ...
    for (c = 0; text[c] != '\0'; c++) {
        ...
        vram = ((uint32_t *) psvDebugScreenFrameBuf.base) + psvDebugScreenCoordX +
