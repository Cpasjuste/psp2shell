...
int p2s_moduleInfoForPid(SceUID pid, SceUID uid) {
    ...
    int res = ksceKernelGetModuleInfo(pid, uid, &moduleInfo);
    if (res == 0) {
...
int p2s_moduleListForPid(SceUID pid) {
    ...
    int res = ksceKernelGetModuleList(pid, 0xFF, 1, ids, &count);
    if (res != 0) {
        ...
        for (int i = 0; i < count; i++) {
            if (ids[i] > 0) {
                memset(&moduleInfo, 0, sizeof(SceKernelModuleInfo));
                moduleInfo.size = sizeof(SceKernelModuleInfo);
                res = ksceKernelGetModuleInfo(pid, ids[i], &moduleInfo);
                if (res == 0) {
